...
/*
    ...
    - 두 테이블의 조인하는 컬럼의 값이 일치하는 행들만 조인되서 조회 -> 일치하지 않는값은 제외
    6. OUTER JOIN: 데이터가 NULL이라도 나오는 것.
    기준테이블의 데이터가 모두 조회되고 조인 테이블에서 조인컬럼의 일치하지 않는 행도 조인해줌.
    ...
    * 순위 매기는 함수(WINDOW FUNCTION)
    ...
    - RANK() OVER(ORDER BY SALARY ASC): 동일한 순위 이후의 등수를 동일한 인원수 만큼 건너뛰고 순위 계산
