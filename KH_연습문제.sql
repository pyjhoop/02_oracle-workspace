...
SELECT EMP_NAME, '19'||SUBSTR(EMP_NO,1,2) AS "생년", SUBSTR(EMP_NO,3,2) AS "생월",
...
EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD')) AS "나이(만)"
FROM EMPLOYEE;
