...
, ROUND(((SALARY+SALARY*NVL(BONUS,0))*12-(SALARY*12*0.03))) AS "실수령액"
FROM EMPLOYEE;
...
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D9','D5') AND HIRE_DATE < '020101';
...
--15. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여 하이유의 생년, 생월, 생일 조회
...
SELECT EMP_NAME, ROUND(ABS(HIRE_DATE - SYSDATE)) AS "근무일수1" , ROUND(SYSDATE - HIRE_DATE) AS "근무일수2"
FROM EMPLOYEE;
...
SELECT EMP_NAME, DEPT_CODE , TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,6)),'RR"년" MM"월" DD"일"') AS "생년월일", 
FLOOR((SYSDATE-TO_DATE(SUBSTR(EMP_NO,1,6)))/365) AS "나이(만)"
FROM EMPLOYEE;
--선생님 풀이
...
SELECT DEPT_CODE, DECODE(DEPT_CODE,'D5','총무부','D6','기획부','D9','영업부')
FROM EMPLOYEE
